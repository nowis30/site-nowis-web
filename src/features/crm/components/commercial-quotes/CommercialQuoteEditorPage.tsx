      ...
      <div className="rounded-[1.5rem] border border-slate-800 bg-slate-900/70 p-6">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            {mode === 'detail' ? <div className="mt-2"><StatusBadge value={form.status} /></div> : null}
          </div>
        ...
        <div className="space-y-6">
          <section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              ...
              {mode === 'detail' ? (
                <label><span className="mb-1 block text-xs text-slate-400">Statut</span><select value={form.status} onChange={(e) => setForm((c) => ({ ...c, status: e.target.value }))} className="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-100"><option>DRAFT</option><option>SENT</option><option>ACCEPTED</option><option>DECLINED</option><option>EXPIRED</option><option>ARCHIVED</option><option>CONVERTED</option></select></label>
              ) : null}
        ...
        <div className="space-y-6">
          ...
          <section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
            ...
            <div className="mt-4 grid gap-2">
              <button type="button" onClick={() => void save()} disabled={loading} className="rounded-lg border border-primary-500/50 bg-primary-500/10 px-3 py-2 text-left text-sm text-primary-200 hover:text-white disabled:opacity-60">{loading ? 'Traitement…' : mode === 'create' ? 'Créer la soumission' : 'Modifier'}</button>
              {mode === 'detail' ? (
                <>
                  ...
                  <button type="button" onClick={() => void archive()} disabled={loading} className="rounded-lg border border-amber-700/50 px-3 py-2 text-left text-sm text-amber-200 disabled:opacity-50">Archiver</button>
                ...
                <button type="button" disabled className="rounded-lg border border-slate-700 px-3 py-2 text-left text-sm text-slate-500">Conversion en facture à finaliser.</button>
              )}
