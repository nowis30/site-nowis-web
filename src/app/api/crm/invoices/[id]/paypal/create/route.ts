...
import { createPayPalInvoiceFromCrmInvoice, getPayPalDiagnostics, serializePayPalApiError } from '@/lib/server/paypal';
...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (missingIssuer.length > 0 || missingCustomer.length > 0) {
    ...
    return NextResponse.json(
      {
        error: 'Facturation incomplete. Complete le profil emetteur et les informations client avant creation PayPal.',
        missingIssuer,
        missingCustomer,
